...
try:
    ...
    if len(uuids.strip()) == 0:
        raise DcpException(ExitCode.EXIT_ERROR, '传入的唯一值为空')
    util.debug("httpUrl:" + httpUrl)
    util.debug("modelName:" + modelName)
    util.debug("uuid:" + uuids)

    ## 启动模型程序 ##
    util.info('停止模型程序 ' + modelName + " " + uuids)
    request = {
        'modelName': modelName,
        'type': 'batch',
        'entity': 'stop',
    ...
    if status != 'SUCCESS':
        raise DcpException(ExitCode.EXIT_ERROR, '请求停止模型程序' + modelName + ' ' + uuids + '失败\n错误信息如下：\n' + message)
    resultCode = data['resultCode'].__str__()
    if resultCode == '00001':  # 停止中
        util.exit(ExitCode.EXIT_SUCCESS, '停止中!')
    elif resultCode == '00002':  # 停止成功
        util.exit(ExitCode.EXIT_SUCCESS, '停止成功!')

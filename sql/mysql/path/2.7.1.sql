...
-- 往服务注册信息表中添加备注字段
ALTER TABLE RC_SERVICE ADD NOTE VARCHAR(4000);
COMMENT ON COLUMN RC_SERVICE.NOTE IS '备注';

-- 往服务授权信息表中添加日期类型、开始时间、结束时间字段
ALTER TABLE RC_USER_SERVICE ADD DATE_TYPE VARCHAR(32) default 'ALL';
COMMENT ON COLUMN RC_USER_SERVICE.DATE_TYPE IS '日期窗口类型（ALL、MON-FRI、WEEKEND、...）';
ALTER TABLE RC_USER_SERVICE ADD START_TIME VARCHAR(32);
COMMENT ON COLUMN RC_USER_SERVICE.START_TIME IS '时间窗口的开始时间';
ALTER TABLE RC_USER_SERVICE ADD END_TIME VARCHAR(32);

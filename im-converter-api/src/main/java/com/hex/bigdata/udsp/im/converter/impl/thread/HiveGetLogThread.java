...
public class HiveGetLogThread extends Thread {
    ...
    public HiveGetLogThread(String id) {
        this.batchService = (BatchJobService) WebApplicationContextUtil.getBean("batchJobService");
        this.id = id;
    }
    ...
    public void run() {
        HiveStatement hiveStatement = HiveJdbcUtil.getHiveStatement(id);
    ...
    private void parseLog(String log) {
        ...
        if (oldPercent != percent) {
            ...
            // 百分比值写入内存中
            if (this.batchService != null && StringUtils.isNoneBlank(id)) {
                this.batchService.building(id, percent);
            }
            oldPercent = percent;
        }
    }

...
public class RealtimeJobService {
        ...
        synchronized (key.intern ()) { // 单节点上锁（主要防止多线程并发资源不同步问题）
            ...
            try {
                ...
                for (RealtimeTotalInfo totalInfo : realtimeTotalInfos) {
                    ...
                    String consumerCronExpression = model.gainConsumerCronExpression ();
                    ...
                    if (RealtimeStatus.READY_START == status) { // 准备启动
                        ...
                        if (HOST_KEY.equals (startHost)) {
                            ...
                            if (flg) {
                                realtimeTotalService.runFail (id);
                            }
                        }
                        // 所有节点操作
    ...
    private RealtimeTotalInfoDto infoToDto(RealtimeTotalInfo info) {
        ...
        if (startTime != null) {
            dto.setStartTime (format.format (startTime));
        }
        Date stopTime = info.getStopTime ();
        if (stopTime != null) {
            dto.setStopTime (format.format (stopTime));
        }
        Date runTime = info.getRunTime ();
        if (runTime != null) {
            dto.setRunTime (format.format (runTime));
        }
        Date updateTime = info.getUpdateTime ();
        if (updateTime != null) {
            dto.setUpdateTime (format.format (updateTime));
        }
        ...
        if (modelFilterCols != null) {
            dto.setRequestContent (JSONUtil.parseList2JSON (modelFilterCols));
        }
        return dto;
    }
